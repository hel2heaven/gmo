...
LogPath=$Path/logs
dtCmd="date '+ %D %T'"
logFile="$1.`date '+%d%b%g'`"
SQL_command_Action >/home/scripts/GMO_PROD/Business_Process/html_files/"$1"_temp.html 2>&1
sed '1d;2d;$d' /home/scripts/GMO_PROD/Business_Process/html_files/"$1"_temp.html > /home/scripts/GMO_PROD/Business_Process/html_files/"$1".html
emailHTML=/home/scripts/GMO_PROD/Business_Process/html_files/"$1".html


send_mail_success()
{

  subject="[GMO PP][SUCCESS]["$bp" is Started]"
...
if [[ $(cat /home/scripts/GMO_PROD/Business_Process/html_files/"$1".html | wc -w) -gt 86 ]];
then
          send_mail_success >> $LogPath/$logFile 2>&1
else
        mailx -s "[GMO PP][ALERT][$bold$bp$normal is Failed]" -r gmo_prod_alert $recipient_tckt < /home/scripts/GMO_PROD/Business_Process/Mail_Body/gmo_prod.txt
fi
